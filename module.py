...
def generate_table(t_vals, voltage_data, voltage_map, table_filename):
    ...
    with PdfPages(table_filename) as pdf:
        for page_num in range(num_pages):
            ...
            ax.axis('off')
            # For multi-page tables, only show headers on the first page
            current_headers = headers if page_num == 0 else None
            table = ax.table(cellText=page_data, colLabels=current_headers, loc='center', cellLoc='center')
            table.auto_set_font_size(False); table.set_fontsize(8); table.scale(1.0, 1.0)
            if current_headers: # Adjust layout only if headers are present
                plt.title('Voltage Threshold Crossing Times', y=0.95)
...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename):
    ...
    table_data = [[vinfo['capacitor'], f"{max_voltages[vkey]:.4f}", f"{max_times[vkey]:.2f}"] for vkey, vinfo in voltage_map.items()]
    print("\nMaximum Voltages and Times:\n" + tabulate(table_data, headers=headers, tablefmt="grid"))
    with PdfPages(table_filename) as pdf:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))
        ax.axis('off')
        ax.set_title("Maximum Voltages and Times", fontweight="bold")
...
def display_interactive_widgets(t_vals, voltage_data, voltage_map):
    ...
    def on_mode_change(change):
        # ******** خطای قبلی در اینجا بود ********
        # از change['new'] به جای change.new استفاده می‌کنیم
