...
def display_interactive_widgets(t_vals, voltage_data, voltage_map, decimal_places):
    ...
    def on_button_clicked(b):
        ...
        if mode_selector.value == 'Voltage to Time':
            ...
            # محاسبه دقیق ولتاژ در زمان مشخص با استفاده از inverse Laplace transform
            try:
                result_voltage = calculate_voltage_at_t(time_target, capacitor_key)
                output_label.value = f"Voltage of {capacitor_name} at {time_target:.{decimal_places}f} s: {result_voltage:.{decimal_places}f} V"
            except Exception as e:
                # در صورت خطا، از interpolation استفاده می‌کنیم
...
def generate_and_display_outputs(t_vals, voltage_data, max_voltages, max_times, voltage_map, decimal_places, start_time_total):
