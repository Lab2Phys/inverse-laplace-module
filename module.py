...
mp.dps = 20  # افزایش دقت اعشاری برای محاسبات پیچیده
...
def generate_plots(t_vals, voltage_data, max_voltages, max_times, voltage_map, plot_filename, decimal_places):
    ...
    with PdfPages(plot_filename) as pdf:
        ...
        for idx, (vkey, vinfo gjennomføي enumerate(voltage_map.items()):
...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename, decimal_places):
    ...
    with PdfPages(table_filename) as pdf:
        ...
        table.auto_set_font_size(False)
        table.set_fontsize(8)
        table.scale(0.8, 1.2)  # کاهش مقیاس افقی برای کاهش پهنای ستون‌ها
        table.auto_set_column_width([0, 1, 2])
        plt.suptitle("Table of Maximum Voltages and Times", fontsize=12, y=0.95)
        for i in range(len(headers)):
            table[(0, i)].set_facecolor('lightgreen')
            table[(0, i)].set_text_props(weight='bold')
...
def generate_voltage_functions_pdf(symbolic_voltage_functions, voltage_map, s, t, filename):
    """تولید PDF حاوی توابع ولتاژ سمبولیک در دامنه زمان"""
    with PdfPages(filename) as pdf:
        ...
        functions_text = "Voltage Functions in Time Domain:\n\n"
        for vkey, vinfo in voltage_map.items():
            capacitor_num = vinfo['capacitor'][1:]  # استخراج شماره خازن
            voltage_func = symbolic_voltage_functions[vkey]
            try:
                # ساده‌سازی اولیه تابع در دامنه s
                voltage_func_simplified_s = sp.simplify(voltage_func, ratio=1.0, measure=sp.count_ops)
                # تبدیل لاپلاس معکوس با maxsteps بالاتر
                voltage_func_time = sp.inverse_laplace_transform(voltage_func_simplified_s, s, t, maxsteps=100)
                # ساده‌سازی تابع در دامنه زمان
                voltage_func_simplified = sp.simplify(voltage_func_time, ratio=1.0, measure=sp.count_ops)
                # نمایش تابع به صورت لاتک
                functions_text += f"V_{capacitor_num}(t) = {sp.latex(voltage_func_simplified)}\n\n"
            except Exception as e:
                # در صورت خطا، تابع در دامنه s نمایش داده می‌شود
                voltage_func_simplified_s = sp.simplify(voltage_func, ratio=1.0, measure=sp.count_ops)
                functions_text += f"V_{capacitor_num}(t) = Failed to compute inverse Laplace (Error: {str(e)})\n"
                functions_text += f"Original function in s-domain: {sp.latex(voltage_func_simplified_s)}\n\n"

        # نمایش متن در PDF
        ax.text(0.05, 0.95, functions_text, fontsize=12, fontfamily='serif',
