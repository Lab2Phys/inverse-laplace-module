...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename, decimal_places):
    ...
    table_data = [[vinfo['capacitor'],
                   f"{max_voltages[vkey]:.{decimal_places}f}",
    ...
    with PdfPages(table_filename) as pdf:
        ...
        # افزودن عنوان
        ax.set_title("Table of maximum voltages and times", fontsize=14, pad=20)
        ...
        table.scale(1.0, 1.5) # افزایش ارتفاع ردیف‌ها
...
def generate_voltage_functions_pdf(symbolic_voltage_functions, voltage_map, s, t, filename):
    ...
    with PdfPages(filename) as pdf:
        ...
        for vkey, vinfo in voltage_map.items():
            capacitor_num = vinfo['capacitor'][1:]
            
            # 1. تابع ولتاژ را از دامنه s به دامنه t تبدیل می‌کنیم
            try:
                voltage_func_s = symbolic_voltage_functions[vkey]
                voltage_func_t = sp.inverse_laplace_transform(voltage_func_s, s, t)
                
                # 2. تابع حاصل را ساده‌سازی می‌کنیم
                voltage_func_simplified = sp.simplify(voltage_func_t)
                
                functions_text += f"V_{capacitor_num}(t) = {sp.latex(voltage_func_simplified)}\n\n"
            except Exception as e:
                functions_text += f"V_{capacitor_num}(t) = Error calculating inverse Laplace transform: {e}\n\n"
