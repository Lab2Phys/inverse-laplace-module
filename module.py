...
def display_interactive_widgets(t_vals, voltage_data, voltage_map, decimal_places):
    ...
    # ایجاد یک باکس برای نمایش نتیجه با فونت درشت و bold
    output_box = widgets.Output(layout={'border': '1px solid black', 'padding': '10px'})
        ...
        with output_box:
            ...
            if mode_selector.value == 'Voltage to Time':
                ...
                if len(indices) > 0:
                    ...
                    print(f"\n\n<span style='font-size: 16px; font-weight: bold;'>Time for {capacitor_name} to reach {voltage_target:.{decimal_places}f} V: {result_time:.{decimal_places}f} s</span>")
                else:
                    print(f"\n\n<span style='font-size: 16px; font-weight: bold;'>{capacitor_name} never reaches {voltage_target:.{decimal_places}f} V</span>")
            else:
                time_target = time_input.value
                if not t_vals.any() or time_target < t_vals[0] or time_target > t_vals[-1]:
                    print(f"\n\n<span style='font-size: 16px; font-weight: bold;'>Time must be between {t_vals[0]:.{decimal_places}f}s and {t_vals[-1]:.{decimal_places}f}s</span>")
                    return
                idx = np.argmin(np.abs(t_vals - time_target))
                result_voltage = voltage_data[capacitor_key][idx]
                print(f"\n\n<span style='font-size: 16px; font-weight: bold;'>Voltage of {capacitor_name} at {t_vals[idx]:.{decimal_places}f} s: {result_voltage:.{decimal_places}f} V</span>")
...
def compute_all_voltages_and_generate_outputs(num_loops, edges, loops, capacitor_branches_map, voltage_map, V, r, s,
                                             decimal_places=4, t_start=0.1, t_end=2000, num_points=2000):
    """تابع اصلی که همه محاسبات و تولید خروجی‌ها را انجام می‌دهد"""
    ...
    # تولید نقاط زمانی
    t_vals = generate_time_points(t_start, t_end, num_points)
    ...
    # نمایش ویجت‌های تعاملی
    display_interactive_widgets(t_vals, voltage_data, voltage_map, decimal_places)
    
    # بازگرداندن اطلاعات فایل‌های تولید شده
    output_files = {}
    for filename in [plot_filename, table_filename, max_voltage_table_filename]:
        if os.path.exists(filename):
            output_files[filename] = os.path.getsize(filename)
