...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename, decimal_places):
    ...
    with PdfPages(table_filename) as pdf:
        ...
        # Reduce table width and increase cell height
        table.scale(0.8, 1.8)
...
def generate_voltage_functions_pdf(symbolic_voltage_functions, voltage_map, s, t, filename):
    # This function has been removed - voltage function PDFs are no longer generated
    pass
...
def display_interactive_widgets(t_vals, voltage_data, voltage_map, decimal_places, voltage_functions=None):
    ...
    # حل مشکل اول: تغییر dropdown به FloatText برای ورود دستی ولتاژ
    voltage_input = widgets.FloatText(value=1.0, description='Voltage (V):', step=0.1, min=0.0)
    ...
    output_label = widgets.Label(value='', style={'font_size': '16px', 'font_weight': 'bold'})
    output_box = widgets.VBox([output_label], layout={'border': '1px solid black', 'padding': '10px', 'width': 'fit-content'})
    
    voltage_box = widgets.HBox([voltage_input])
    ...
    def on_button_clicked(b):
        ...
        if mode_selector.value == 'Voltage to Time':
            voltage_target = voltage_input.value
            ...
            # حل مشکل دوم: محاسبه دقیق ولتاژ در زمان مشخص
            ...
            # محاسبه دقیق ولتاژ در زمان مشخص با inverse Laplace transform
            try:
                if voltage_functions is not None and capacitor_key in voltage_functions:
                    # محاسبه مستقیم با inverse Laplace transform
                    s_func = voltage_functions[capacitor_key]
                    
                    def s_func_for_laplace(s_val):
                        return mp.mpc(s_func(complex(s_val)))
                    
                    if time_target == 0:
                        result_voltage = 0.0
                    else:
                        val = invertlaplace(s_func_for_laplace, time_target, method='talbot', degree=8)
                        result_voltage = float(val.real)
                    
                    output_label.value = f"Voltage of {capacitor_name} at {time_target:.{decimal_places}f} s: {result_voltage:.{decimal_places}f} V (exact)"
                else:
                    # fallback به interpolation
                    result_voltage = np.interp(time_target, t_vals, voltage_data[capacitor_key])
                    output_label.value = f"Voltage of {capacitor_name} at {time_target:.{decimal_places}f} s: {result_voltage:.{decimal_places}f} V (interpolated)"
                    
            except Exception as e:
                print(f"Error in calculation: {e}")  # برای debug
                # fallback به interpolation در صورت خطا
                result_voltage = np.interp(time_target, t_vals, voltage_data[capacitor_key])
                output_label.value = f"Voltage of {capacitor_name} at {time_target:.{decimal_places}f} s: {result_voltage:.{decimal_places}f} V (interpolated - error in exact calculation)"

    calculate_button.on_click(on_button_clicked)
    on_mode_change({'new': mode_selector.value})
    print("\nSelect mode, capacitor, and value, then click Calculate:")
    display(widgets.VBox([mode_selector, capacitor_dropdown, voltage_box, time_box, calculate_button, output_box]))

def generate_and_display_outputs(t_vals, voltage_data, max_voltages, max_times, voltage_map, decimal_places, start_time_total, voltage_functions=None):
    # Define output file names
    plot_filename = "plots.pdf"
    table_filename = "tables.pdf"
    ...
    display_interactive_widgets(t_vals, voltage_data, voltage_map, decimal_places, voltage_functions)

    return plot_filename, table_filename, max_voltage_table_filename