...
def display_interactive_widgets(t_vals, voltage_data, voltage_map, decimal_places, voltage_functions=None):
    ...
    def on_button_clicked(b):
        ...
        if mode_selector.value == 'Voltage to Time':
            voltage_target = voltage_input.value  # تغییر از voltage_dropdown.value
            ...
            # محاسبه دقیق ولتاژ در زمان مشخص
            try:
                if voltage_functions is not None:
                    # استفاده از inverse Laplace transform برای محاسبه دقیق
                    result_voltage = calculate_single_voltage_at_t(time_target, capacitor_key, voltage_functions)
                    output_label.value = f"Voltage of {capacitor_name} at {time_target:.{decimal_places}f} s: {result_voltage:.{decimal_places}f} V (exact)"
                else:
                    # از interpolation دقیق‌تر استفاده می‌کنیم که بر اساس داده‌های محاسبه شده است
                ...
                output_label.value = f"Error calculating voltage: {str(e)}"
...
def calculate_single_voltage_at_t(t_val, voltage_key, voltage_functions):
    """تابع جدید برای محاسبه دقیق ولتاژ در زمان مشخص بدون وابستگی به متغیرهای گلوبال"""
    if t_val == 0:
        return 0.0
    try:
        s_func_internal = voltage_functions[voltage_key]
        def s_func_for_laplace(s_laplace):
            return mp.mpc(s_func_internal(complex(s_laplace)))
        val = invertlaplace(s_func_for_laplace, t_val, method='talbot', degree=8)
        return float(val.real)
    except Exception as e:
        print(f"Error in Laplace transform calculation: {e}")
        return 0.0
