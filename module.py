...
def generate_plots(t_vals, voltage_data, max_voltages, max_times, voltage_map, plot_filename):
    """تولید فایل PDF شامل نمودارها با آرایش پویا."""
    num_plots = len(voltage_map)
    # تعیین تعداد سطر و ستون به صورت پویا
    ncols = 2
...
def generate_table(t_vals, voltage_data, voltage_map, table_filename):
    """تولید فایل PDF شامل جدول زمان عبور از آستانه ولتاژ."""
    ...
    with PdfPages(table_filename) as pdf:
        for page in range(0, total_rows, rows_per_page):
            ...
            table.auto_set_font_size(False); table.set_fontsize(8); table.scale(1.0, 0.9)
            fig.subplots_adjust(left=0.15, right=0.85, top=0.92, bottom=0.08)
            pdf.savefig(fig, bbox_inches='tight', dpi=300)
            plt.close(fig)

def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename):
    """تولید فایل PDF شامل جدول ولتاژ و زمان ماکزیمم."""
    headers = ["Capacitor", "Max Voltage (V)", "Time of Max Voltage (s)"]
    table_data = []
    for vkey, vinfo in voltage_map.items():
        row = [
            vinfo['capacitor'],
            f"{max_voltages[vkey]:.4f}",
            f"{max_times[vkey]:.2f}"
        ]
        table_data.append(row)
...
def generate_outputs(t_vals, voltage_data, max_voltages, max_times, voltage_map, 
                     ...
                     max_voltage_table_filename="max_voltages_table.pdf"):
    """تولید تمام خروجی‌های PDF (نمودار و جداول)."""
    try:
        ...
        generate_max_voltage_table(max_voltages, max_times, voltage_map, max_voltage_table_filename)
    except (IOError, PermissionError) as e:
        print(f"خطا در نوشتن فایل‌های خروجی: {e}")
        raise

def display_interactive_widgets(t_vals, voltage_data, voltage_map):
    """نمایش ویجت‌های تعاملی برای انتخاب خازن، ولتاژ یا زمان."""
    # ویجت‌ها
    ...
    mode_selector = widgets.RadioButtons(
        options=['Voltage to Time', 'Time to Voltage'],
        description='Mode:',
        disabled=False
    )
    ...
    volt_step = 0.1
    v_levels = np.arange(volt_step, max_voltage_all + volt_step, volt_step)
    voltage_options = [(f"{v:.2f} V", v) for v in v_levels]
    voltage_dropdown = widgets.Dropdown(options=voltage_options, description='Voltage:', value=v_levels[0])
    ...
    # تابع به‌روزرسانی نمایش ویجت‌ها
    ...
    def on_button_clicked(b):
        ...
        if mode_selector.value == 'Voltage to Time':
            ...
            if len(indices) > 0:
                ...
                output_label.value = f"Time for {capacitor_name} to reach {voltage_target:.2f} V: {result_time:.3f} s"
            else:
                output_label.value = f"{capacitor_name} never reaches {voltage_target:.2f} V"
        else: # Time to Voltage
            time_target = time_input.value
            if time_target < t_vals[0] or time_target > t_vals[-1]:
                 output_label.value = f"Time must be between {t_vals[0]:.2f}s and {t_vals[-1]:.2f}s"
                 return
            # یافتن نزدیک‌ترین ایندکس به زمان ورودی
            idx = np.argmin(np.abs(t_vals - time_target))
            result_voltage = voltage_data[capacitor_key][idx]
            output_label.value = f"Voltage of {capacitor_name} at {t_vals[idx]:.2f} s: {result_voltage:.4f} V"
