...
def setup_circuit_parameters(circuit_params):
    """تنظیم پارامترهای مدار"""
    s, t = sp.symbols('s t', real=True, positive=True)
    
    R = circuit_params['R']
    C = circuit_params['C']
    V = circuit_params['V']  # دریافت V از پارامترهای ورودی
    
    r = 1 / (C * s)
    
    # ایجاد لیست edges با impedance
    edges = []
    capacitor_edges = {(1, 2), (1, 6), (2, 7), (4, 5), (6, 7)}
    
    for edge in circuit_params['edges']:
        if edge in capacitor_edges:
            edges.append((edge[0], edge[1], R + r))
        else:
            edges.append((edge[0], edge[1], R))
    ...
    voltage_map = {
        ...
        'v5': {'label': 'Voltage V5', 'capacitor': 'C5', 'index': 4},
    }
    
    return s, t, r, edges, voltage_map, V
...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename, decimal_places):
    headers = ["Capacitor", "Max Voltage (V)", "Max Time (s)"]
    ...
    with PdfPages(table_filename) as pdf:
        ...
        # اضافه کردن عنوان جدول
        fig.suptitle('Table of maximum voltages and times', fontsize=14, fontweight='bold', y=0.95)
        
        table = ax.table(cellText=table_data, colLabels=headers, loc='center', cellLoc='center')
        table.auto_set_font_size(False)
        table.set_fontsize(10)
        # کاهش پهنای جدول و افزایش ارتفاع سلول‌ها
        table.scale(0.8, 1.8)
...
def generate_voltage_functions_pdf(symbolic_voltage_functions, voltage_map, s, t, filename):
    """این تابع حذف شده است - PDF توابع ولتاژ دیگر تولید نمی‌شود"""
    pass
...
def display_interactive_widgets(t_vals, voltage_data, voltage_map, decimal_places):
    ...
    def on_button_clicked(b):
        output_label.value = '<div style="font-size: 18px; font-weight: bold; color: #2E86AB; padding: 10px; background-color: #F0F8FF; border-radius: 5px; margin-top: 10px;">Calculating...</div>'
        ...
        if mode_selector.value == 'Voltage to Time':
            ...
            if len(indices) > 0:
                ...
                output_label.value = f'<div style="font-size: 20px; font-weight: bold; color: #28A745; padding: 15px; background-color: #E8F5E8; border: 2px solid #28A745; border-radius: 8px; margin-top: 15px; text-align: center;">Time for {capacitor_name} to reach {voltage_target:.{decimal_places}f} V: <span style="color: #D63384;">{result_time:.{decimal_places}f} s</span></div>'
            else:
                output_label.value = f'<div style="font-size: 18px; font-weight: bold; color: #DC3545; padding: 15px; background-color: #FCE4EC; border: 2px solid #DC3545; border-radius: 8px; margin-top: 15px; text-align: center;">{capacitor_name} never reaches {voltage_target:.{decimal_places}f} V</div>'
        else:
            time_target = time_input.value
            if not t_vals.any() or time_target < t_vals[0] or time_target > t_vals[-1]:
                output_label.value = f'<div style="font-size: 18px; font-weight: bold; color: #FD7E14; padding: 15px; background-color: #FFF3CD; border: 2px solid #FD7E14; border-radius: 8px; margin-top: 15px; text-align: center;">Time must be between {t_vals[0]:.{decimal_places}f}s and {t_vals[-1]:.{decimal_places}f}s</div>'
                return
            idx = np.argmin(np.abs(t_vals - time_target))
            result_voltage = voltage_data[capacitor_key][idx]
            output_label.value = f'<div style="font-size: 20px; font-weight: bold; color: #28A745; padding: 15px; background-color: #E8F5E8; border: 2px solid #28A745; border-radius: 8px; margin-top: 15px; text-align: center;">Voltage of {capacitor_name} at {t_vals[idx]:.{decimal_places}f} s: <span style="color: #D63384;">{result_voltage:.{decimal_places}f} V</span></div>'
...
def run_complete_analysis(circuit_params, analysis_params):
    """اجرای تحلیل کامل مدار"""
    print("Starting circuit analysis...")
    start_time_total = time.time()
    
    # تنظیم پارامترهای مدار
    s, t, r, edges, voltage_map, V = setup_circuit_parameters(circuit_params)
    
    # ایجاد ماتریس امپدانس
    Z, loop = create_symbolic_impedance_matrix(
        circuit_params['num_loops'], 
        edges, 
        circuit_params['loops'], 
        s
    )
    
    # ایجاد توابع ولتاژ
    voltage_functions, symbolic_voltage_functions = create_precompiled_voltage_functions(
        Z, V, edges, loop, circuit_params['capacitor_branches'], r, s
    )
    
    # تولید نقاط زمانی
    t_vals = generate_time_points(
        analysis_params['t_start'], 
        analysis_params['t_end'], 
        analysis_params['num_points']
    )
    
    # محاسبه ولتاژها
    voltage_data, max_voltages, max_times = compute_voltages(
        t_vals, voltage_functions, voltage_map, analysis_params['decimal_places']
    )
    
    # تولید خروجی‌ها
    generate_outputs(
        t_vals, voltage_data, max_voltages, max_times, voltage_map,
        plot_filename=analysis_params['plot_filename'],
        table_filename=analysis_params['table_filename'],
        max_voltage_table_filename=analysis_params['max_voltage_table_filename'],
        decimal_places=analysis_params['decimal_places']
    )
    
    # تولید PDF توابع ولتاژ (غیرفعال شده)
    generate_voltage_functions_pdf(
        symbolic_voltage_functions, voltage_map, s, t, 
        analysis_params['voltage_functions_filename']
    )
    
    # نمایش ویجت تعاملی
    display_interactive_widgets(t_vals, voltage_data, voltage_map, analysis_params['decimal_places'])
    
    # گزارش نهایی
    print(f"\nReports generated:")
    for filename_key in ['plot_filename', 'table_filename', 'max_voltage_table_filename', 'voltage_functions_filename']:
        filename = analysis_params[filename_key]
        if os.path.exists(filename):
            file_type = filename_key.replace('_filename', '').replace('_', ' ').title()
            print(f"{file_type}: {filename} | Size: {os.path.getsize(filename)/1024:.2f} KB")
