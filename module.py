...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename, decimal_places):
    ...
    with PdfPages(table_filename) as pdf:
        ...
        ax.text(0.5, 0.85, 'Table of Maximum Voltages and Times', 
                fontsize=16, fontweight='bold', ha='center', transform=ax.transAxes)
        
        # ایجاد جدول با bbox مستقیم
        table = ax.table(cellText=table_data, colLabels=headers, 
                        loc='center', cellLoc='center',
                        bbox=[0.15, 0.4, 0.7, 0.3])  # [x, y, width, height]
        ...
        for i in range(len(headers)):
            ...
            table[(0, i)].set_height(0.12)  # ارتفاع بیشتر برای هدر
        
        # تنظیم پهنای و ارتفاع سلول‌های داده
        for i in range(1, len(table_data) + 1):  # ردیف‌های داده
            for j in range(len(headers)):  # ستون‌ها
                ...
                table[(i, j)].set_height(0.10)  # ارتفاع بیشتر برای هر ردیف
