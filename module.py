...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename, decimal_places):
    """تولید جدول ماکزیمم ولتاژها و زمان‌ها با بهبودهای ظاهری"""
    headers = ["Capacitor", "Max Voltage (V)", "Time of Max Voltage (s)"]
    table_data = [[vinfo['capacitor'], f"{max_voltages[vkey]:.{decimal_places}f}", 
                   f"{max_times[vkey]:.{decimal_places}f}"] for vkey, vinfo in voltage_map.items()]
    ...
    with PdfPages(table_filename) as pdf:
        ...
        ax.text(0.5, 0.9, 'Table of Maximum Voltages and Times', 
        ...
        # تنظیم اندازه جدول - کاهش پهنا و افزایش ارتفاع
        table.scale(0.8, 2.0)  # پهنا: 0.8 (کمتر)، ارتفاع: 2.0 (بیشتر)
        
        # رنگ‌آمیزی هدر سبز و تنظیمات استایل
        for i in range(len(headers)):
            table[(0, i)].set_facecolor('lightgreen')
            table[(0, i)].set_text_props(weight='bold')
            # کاهش پهنای سلول‌های هدر
            table[(0, i)].set_width(0.25)  # پهنای کمتر برای هر ستون
        
        # تنظیم پهنای سلول‌های داده
        for i in range(1, len(table_data) + 1):  # ردیف‌های داده
            for j in range(len(headers)):  # ستون‌ها
                table[(i, j)].set_width(0.25)  # پهنای کمتر برای هر ستون
                table[(i, j)].set_height(0.08)  # ارتفاع بیشتر برای هر ردیف
        
        # تنظیم موقعیت جدول
        table.set_bbox([0.1, 0.4, 0.8, 0.4])  # [x, y, width, height]
...
def generate_voltage_functions_pdf(symbolic_voltage_functions, voltage_map, s, t, filename):
    ...
    with PdfPages(filename) as pdf:
        ...
        # ایجاد متن برای نمایش توابع
        functions_text = "Time Domain Voltage Functions:\n\n"
        ...
        for vkey, vinfo in voltage_map.items():
            ...
            try:
                # تبدیل از حوزه s به حوزه زمان با استفاده از تبدیل معکوس لاپلاس
                print(f"Converting {vinfo['capacitor']} to time domain...")
                
                # ساده‌سازی تابع در حوزه s قبل از تبدیل
                ...
                # ساده‌سازی بیشتر تابع زمانی
                voltage_func_t_simplified = sp.simplify(voltage_func_t)
                
                # اگر تابع همچنان پیچیده است، تلاش برای ساده‌سازی بیشتر
                voltage_func_t_simplified = sp.expand(voltage_func_t_simplified)
                voltage_func_t_simplified = sp.collect(voltage_func_t_simplified, t)
                
                functions_text += f"V_{capacitor_num}(t) = {voltage_func_t_simplified}\n\n"
                
            except Exception as e:
                print(f"Warning: Could not convert {vinfo['capacitor']} to time domain: {e}")
                # در صورت عدم موفقیت، حداقل تابع s را نمایش بده
                voltage_func_s_simplified = sp.simplify(voltage_func_s)
                functions_text += f"V_{capacitor_num}(s) = {voltage_func_s_simplified}\n"
                functions_text += f"[Time domain conversion failed for {vinfo['capacitor']}]\n\n"
        
        # نمایش متن در PDF با فونت کوچکتر برای جا دادن توابع طولانی
        ax.text(0.05, 0.95, functions_text, fontsize=10, fontfamily='serif', 
