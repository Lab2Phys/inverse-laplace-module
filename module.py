...
def generate_max_voltage_table(max_voltages, max_times, voltage_map, table_filename, decimal_places):
    ...
    with PdfPages(table_filename) as pdf:
        ...
        # اضافه کردن عنوان به بالای جدول
        ...
        # کاهش عرض ستون و افزایش ارتفاع سطر
        table.scale(1.0, 1.5) 
...
def generate_voltage_functions_pdf(symbolic_voltage_functions, voltage_map, s, t, filename):
    """
    تولید PDF حاوی توابع ولتاژ.
    این تابع سعی می‌کند تبدیل لاپلاس معکوس را انجام دهد تا V(t) را به دست آورد.
    اگر محاسبه سمبولیک به دلیل پیچیدگی با خطا مواجه شود، V(s) را نمایش می‌دهد.
    """
    ...
    with PdfPages(filename) as pdf:
        ...
        functions_text = r"Voltage Functions:" + "\n\n"
        ...
        for vkey, vinfo in voltage_map.items():
            ...
            try:
                # تلاش برای انجام تبدیل لاپلاس معکوس و ساده‌سازی
                ...
                # در صورت خطا، تابع در حوزه s نمایش داده می‌شود
                error_msg = str(e).replace('{', '\{').replace('}', '\}')
                functions_text += (f"$V_{{{capacitor_num}}}(s) = {sp.latex(sp.simplify(voltage_func_s))}$ \n\n"
                                   f"\\textbf{{(Inverse Laplace transform failed: Calculation too complex)}} \n\n")
