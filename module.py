...
def generate_voltage_functions_pdf(symbolic_voltage_functions, voltage_map, s, t, filename):
    """تولید PDF حاوی توابع ولتاژ سمبولیک"""
    with PdfPages(filename) as pdf:
        fig, ax = plt.subplots(figsize=(8.27, 11.69))
        ax.axis('off')
        
        # ایجاد متن برای نمایش توابع
        functions_text = "Voltage Functions:\n\n"
        for vkey, vinfo in voltage_map.items():
            capacitor_num = vinfo['capacitor'][1:]  # استخراج شماره خازن
            voltage_func = symbolic_voltage_functions[vkey]
            
            # ساده‌سازی تابع
            voltage_func_simplified = sp.simplify(voltage_func)
            functions_text += f"V_{capacitor_num}(t) = {voltage_func_simplified}\n\n"
