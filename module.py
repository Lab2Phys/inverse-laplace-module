...
def generate_plots(t_vals, voltage_data, max_voltages, max_times, voltage_map, plot_filename):
    """تولید فایل PDF شامل نمودارها با آرایش 2x2."""
...
def generate_table(t_vals, voltage_data, voltage_map, table_filename):
    """تولید فایل PDF شامل جدول دوصفحه‌ای با اندازه بهینه."""
...
def display_interactive_widgets(t_vals, voltage_data, voltage_map):
    """نمایش ویجت‌های تعاملی برای انتخاب خازن و ولتاژ."""
    max_voltage_all = max(np.max(np.abs(v)) for v in voltage_data.values())
    volt_step = 0.1
    v_levels = np.arange(volt_step, max_voltage_all + volt_step, volt_step)
    ...
    capacitor_dropdown = widgets.Dropdown(
        options=capacitor_options,
        description='Capacitor:',
        value='v1'
    )
    voltage_dropdown = widgets.Dropdown(
    ...
    calculate_button = widgets.Button(
        ...
        tooltip='Click to calculate crossing time'
    )

    output = widgets.Output()
    time_output = widgets.Label(value='', style={'font_size': '16px'})

    def find_crossing_time(capacitor, voltage):
        with output:
            output.clear_output()
            print(f"Time when {voltage_map[capacitor]['capacitor']} reaches {voltage:.2f} V:")
        v_abs = np.abs(voltage_data[capacitor])
        indices = np.where(v_abs >= voltage)[0]
        if len(indices) > 0:
            time_output.value = f"{t_vals[indices[0]]:.3f} s"
        else:
            time_output.value = "never reaches"

    def on_button_clicked(b):
        find_crossing_time(capacitor_dropdown.value, voltage_dropdown.value)

    calculate_button.on_click(on_button_clicked)
    print("\nSelect capacitor and voltage threshold, then click Calculate:")
    display(widgets.VBox([capacitor_dropdown, voltage_dropdown, calculate_button, output, time_output]))